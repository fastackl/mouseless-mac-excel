-- Mouseless Mac Excel: Hammerspoon bootstrap template.
--
...
-- local clone, e.g. "/Users/yourname/code/mouseless-mac-excel".
--
...
local LOG_PATH = "/tmp/mouseless-mac-excel.log"

local function logf(fmt, ...)
  local line = "[" .. os.date("%Y-%m-%d %H:%M:%S") .. "] " .. string.format(fmt, ...)
  print("[mouseless-mac-excel] " .. line)
...
if not ok then
  hs.alert.show("Mouseless Mac Excel load error: " .. tostring(err), 4)
...
if not _G.__mme_pathwatcher then
  _G.__mme_pathwatcher = hs.pathwatcher.new(PROJECT, function(files)
    for _, f in ipairs(files) do
      if f:match("%.lua$") then
        hs.alert.show("Reloading Mouseless Mac Excel", 0.6)
        hs.reload()
        return
      end
    end
  end)
  _G.__mme_pathwatcher:start()
end

hs.alert.show("Mouseless Mac Excel ready", 0.8)
